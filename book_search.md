    ...
    - Assertions and Assumptions
        * results for string matching are case sensitive
            - 'the' and 'THE' are not equivalent so use `===`
                * `if searchTerm === `
    - Edge Cases
        * words that aren't search term but contain term in it ex. searchTerm of 'the' and string 'then', provided a search term that is empty, a search term that is not found in any of the provided information in json object, an object containing no content, an object containing no books, if user inputs a non string input
...
```
    * provided a search term that is empty, a search term that is not found in any of the provided information in json object, an object containing no content, an object containing no books, if user inputs a non string input
```
